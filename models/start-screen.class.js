class StartScreen extends DrawableObject {
  ...
  showHelpText() {
    const rect = this.canvas.getBoundingClientRect(); // Canvas-Position und Größe abrufen

    const helpOverlay = document.createElement("div");
    helpOverlay.style.position = "absolute"; // Relative zur Canvas-Position
    helpOverlay.style.top = `${rect.top}px`;
    helpOverlay.style.left = `${rect.left}px`;
    helpOverlay.style.width = `${rect.width}px`;
    helpOverlay.style.height = `${rect.height}px`;
    ...
    // Dynamische Schriftgröße basierend auf Canvas-Breite
    const fontSize = Math.max(14, Math.min(20, rect.width / 40));
    helpOverlay.style.fontSize = `${fontSize}px`;
    ...
    <div style="text-align: center; max-height: 90%; overflow-y: auto;">
      ...
      <ul style="list-style: none; padding: 0;">
        ...
        <li><b>Boss:</b> The boss has two phases. During the transition, it is invulnerable. In the second phase, it shoots small chickens.</li>
        <li><b>Bottles:</b> If your bottles are empty, they will respawn in the front half of the level.</li>
      </ul>
  ...
  showImpressumText() {
    ...
    impressumOverlay.style.position = "absolute"; // Relative zur Canvas-Position
    impressumOverlay.style.top = `${rect.top}px`;
    impressumOverlay.style.left = `${rect.left}px`;
    impressumOverlay.style.width = `${rect.width}px`;
    impressumOverlay.style.height = `${rect.height}px`;
    ...
    // Dynamische Schriftgröße basierend auf Canvas-Breite
    const fontSize = Math.max(14, Math.min(20, rect.width / 40));
    impressumOverlay.style.fontSize = `${fontSize}px`;
